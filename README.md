...
- 📝 **日报 / 周报 / 月报** —— 四种模板（标准/简洁/技术/OKR）× 三档详略（简要/标准/详尽）自由组合；详尽档两段式生成（先提工作项清单再逐项扩写），数字一律来自真实时间统计；周报月报优先复用已生成的日报做层级汇总
- ⏰ **定时日报** —— 设定时间自动生成当日日报并系统通知，下班即取，忘不了也不用等
- 📊 **统计页** —— GitHub 风格年度活跃热力图、时段分布（7×24）、连续记录天数；应用记录支持今日/本周/本月/近30天，含占比与每应用首次/最后使用时间
