...
## 13. 统计页「应用记录」模块（v1.4，替换原「应用时长 Top」区）

- 区头：标题「应用记录」+ 右侧 SegmentControl（今日 | 本周 | 本月 | 近30天），默认今日。
- 三枚迷你卡横排（与概览卡同款式但更小）：总应用数 N 个 / 总时长 Xh Ym / 日均 Xh Ym。
- Top 15 水平条形沿用现有样式。
- 明细表：列 = 应用（分类色点 + 名称）/ 时长（右对齐）/ 占比（右对齐，1 位小数 %）/ 首次使用 / 最后使用。今日周期首末显示 HH:mm，其余周期显示「M月D日 HH:mm」。行 hover --surface-2。>15 行折叠，底部居中幽灵按钮「展开全部 N 个应用」。
- 表格外层 overflow-x auto；窄窗口时首末两列可被压缩省略（min-width 断点内隐藏，规格：<720px 隐藏首次列）。

## 14. 报告生成器「详略」维度（v1.4）

- Generator 表单在「模板」下方新增一行「详略」SegmentControl：简要 | 标准 | 详尽（默认读 settings.report.defaultDetail，切换即存）。
- 三档 hover Tooltip 一句话说明：简要「只留最重要的 3-5 件事」/ 标准「均衡的篇幅与细节」/ 详尽「逐项展开，引用具体文件与数据，生成时间约长一倍」。
- 生成中文案：详尽档第一段显示「AI 正在梳理工作项…」，第二段显示「AI 撰写中…」（沿用现有进度事件，仅文案随 detail 变化）。
